...
const char *c_szToolTip[] = {
    ...
    "mailto:rodrigueztech@gmail.com"};

BOOL WINAPI LoadResult(char *strFileName);
BOOL WINAPI SaveResult(char *strFileName, BOOL isSaveSelected);
static void Helper_ParsePortList(char* strList, int startPort, int endPort, BOOL useRange);
static void Helper_WaitAndCleanUp(DWORD_PTR dwStartTime);
void WINAPI StartScan();
void WINAPI ResolveDomain();
void WINAPI ScanThread(int intCurThread);

void WINAPI On_InitDialog(HWND hWndDlg);
...
static INT_PTR CALLBACK MainDlgProc(HWND g_hWnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
static INT_PTR CALLBACK MainDlgProc(HWND hWndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
        ...
        {
            RECT rcGroup = { 8, 72, 0, 0 };
            RECT rcList = { 15, 85, 0, 0 };

            MapDialogRect(hWndDlg, &rcGroup);
            MapDialogRect(hWndDlg, &rcList);

            MoveWindow(GetDlgItem(g_hWnd, IDG_RESULT),
                rcGroup.left,
                rcGroup.top,
                LOWORD(lParam) - (rcGroup.left * 2),
                HIWORD(lParam) - rcGroup.top - 20,
                TRUE);

            MoveWindow(g_hwndListView,
                rcList.left,
                rcList.top,
                LOWORD(lParam) - (rcList.left * 2),
                HIWORD(lParam) - rcList.top - 30,
                TRUE);
            SendMessage(g_hwndStatus, WM_SIZE, wParam, lParam);
            ListView_SetColumnWidth(g_hwndListView, 2, LOWORD(lParam) - 300);
        }
        break;
    case WM_COMMAND:
        On_Command(wParam);
    ...
    case WM_CLOSE:
        ...
        if (RegCreateKeyEx(HKEY_CURRENT_USER, "Software\\BrutePortScanner", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &hKey, NULL) == ERROR_SUCCESS)
        {
            SendDlgItemMessage(g_hWnd, IDIP_STARTIP, IPM_GETADDRESS, 0, (LPARAM)&dwStartIP);
            RegSetValueEx(hKey, "StartIP", 0, REG_DWORD, (LPBYTE)&dwStartIP, sizeof(DWORD));

            SendDlgItemMessage(g_hWnd, IDIP_ENDIP, IPM_GETADDRESS, 0, (LPARAM)&dwStartIP);
            RegSetValueEx(hKey, "EndIP", 0, REG_DWORD, (LPBYTE)&dwStartIP, sizeof(DWORD));

            dwVal = GetDlgItemInt(g_hWnd, IDE_STARTPORT, NULL, 0);
            RegSetValueEx(hKey, "StartPort", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));

            dwVal = GetDlgItemInt(g_hWnd, IDE_ENDPORT, NULL, 0);
            RegSetValueEx(hKey, "EndPort", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));

            dwVal = GetDlgItemInt(g_hWnd, IDE_THREAD, NULL, 0);
            RegSetValueEx(hKey, "ThreadNum", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));

            dwVal = GetDlgItemInt(g_hWnd, IDE_TIMEOUT, NULL, 0);
            RegSetValueEx(hKey, "TimeOut", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));

            dwVal = (GetMenuState(g_hMenu, IDC_GETBANNER, MF_BYCOMMAND) & MF_CHECKED) ? 1 : 0;
            RegSetValueEx(hKey, "GetBanner", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));

            dwVal = (GetMenuState(g_hMenu, IDC_KEEPHISTORY, MF_BYCOMMAND) & MF_CHECKED) ? 1 : 0;
            RegSetValueEx(hKey, "KeepHistory", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));

            dwVal = SendDlgItemMessage(g_hWnd, IDC_USERANGE, BM_GETCHECK, 0, 0) ? 1 : 0;
            RegSetValueEx(hKey, "UseRange", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));

            dwVal = SendDlgItemMessage(g_hWnd, IDC_USELIST, BM_GETCHECK, 0, 0) ? 1 : 0;
            RegSetValueEx(hKey, "UseList", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));

            dwVal = isNoAsk ? 1 : 0;
            RegSetValueEx(hKey, "NoAsk", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));

            GetDlgItemText(g_hWnd, IDE_PORTLIST, strTemp, sizeof(strTemp));
            RegSetValueEx(hKey, "PortList", 0, REG_SZ, (LPBYTE)strTemp, (DWORD)strlen(strTemp) + 1);
...
LRESULT CALLBACK MouseProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    if (nCode >= 0 && (IsChild(g_hWnd, ((MOUSEHOOKSTRUCT *)lParam)->hwnd)))
    {
        ...
        {
            msg.pt = ((MOUSEHOOKSTRUCT*)lParam)->pt;
            msg.time = GetTickCount64();
            msg.lParam = 0;
            msg.wParam = 0;
            msg.message = wParam;
...
static int CALLBACK CompareFunc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
    ...
    if (lParamSort == 0)
    {
        if (p1->dwAddr > p2->dwAddr) res = 1;
        else if (p1->dwAddr < p2->dwAddr) res = -1;
        else {
            if (p1->wPort > p2->wPort) res = 1;
            else if (p1->wPort < p2->wPort) res = -1;
        }
    }
    ...
    {
        if (p1->lpstrBanner && p2->lpstrBanner)
            res = lstrcmpi(p1->lpstrBanner, p2->lpstrBanner);
    }
    return res;
}
...
void WINAPI On_InitDialog(HWND hWndDlg)
{
    ...
    {
        ti.uId = (UINT_PTR)GetDlgItem(g_hWnd, i);
    ...
    for (i = 300; i < 312; i++)
    {
        ...
        if (i == 300)
            SetClassLongPtr(g_hWnd, GCLP_HICON, (LONG_PTR)hIcon);
    ...
    SetWindowLongPtr(GetDlgItem(g_hWnd, IDC_HOME), GWLP_USERDATA, (LONG_PTR)SetWindowLongPtr(GetDlgItem(g_hWnd, IDC_HOME), GWLP_WNDPROC, (LONG_PTR)HyperLinkProc));
    SetWindowLongPtr(GetDlgItem(g_hWnd, IDC_MAIL), GWLP_USERDATA, (LONG_PTR)SetWindowLongPtr(GetDlgItem(g_hWnd, IDC_MAIL), GWLP_WNDPROC, (LONG_PTR)HyperLinkProc));

    HKEY hKey;
    if (RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\BrutePortScanner", 0, KEY_READ, &hKey) == ERROR_SUCCESS)
    {
        DWORD dwType, dwSize, dwVal;
        char strTemp[600];
        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "StartIP", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            SendDlgItemMessage(g_hWnd, IDIP_STARTIP, IPM_SETADDRESS, 0, (LPARAM)dwVal);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "EndIP", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            SendDlgItemMessage(g_hWnd, IDIP_ENDIP, IPM_SETADDRESS, 0, (LPARAM)dwVal);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "StartPort", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            SetDlgItemInt(g_hWnd, IDE_STARTPORT, dwVal, 0);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "EndPort", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            SetDlgItemInt(g_hWnd, IDE_ENDPORT, dwVal, 0);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "ThreadNum", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            SetDlgItemInt(g_hWnd, IDE_THREAD, dwVal, 0);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "TimeOut", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            SetDlgItemInt(g_hWnd, IDE_TIMEOUT, dwVal, 0);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "GetBanner", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            CheckMenuItem(g_hMenu, IDC_GETBANNER, dwVal ? MF_CHECKED : MF_UNCHECKED);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "KeepHistory", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            CheckMenuItem(g_hMenu, IDC_KEEPHISTORY, dwVal ? MF_CHECKED : MF_UNCHECKED);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "UseRange", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            SendDlgItemMessage(g_hWnd, IDC_USERANGE, BM_SETCHECK, dwVal, 0);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "UseList", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            SendDlgItemMessage(g_hWnd, IDC_USELIST, BM_SETCHECK, dwVal, 0);

        dwSize = sizeof(DWORD);
        if (RegQueryValueEx(hKey, "NoAsk", NULL, NULL, (LPBYTE)&dwVal, &dwSize) == ERROR_SUCCESS)
            isNoAsk = (BOOL)dwVal;

        dwSize = sizeof(strTemp);
        if (RegQueryValueEx(hKey, "PortList", NULL, &dwType, (LPBYTE)strTemp, &dwSize) == ERROR_SUCCESS)
        {
            if (dwType == REG_SZ)
...
void WINAPI On_Command(WPARAM wParam)
{
    ...
    case IDE_ENDPORT:
        if ((HIWORD(wParam)) == EN_CHANGE)
        {
            i = GetDlgItemInt(g_hWnd, id, NULL, FALSE);
            if (i > 65535)
                SetDlgItemInt(g_hWnd, id, 65535, 0);
            else if (i && (id == IDE_STARTPORT))
    ...
    case IDE_THREAD:
    case IDE_TIMEOUT:
        if (((HIWORD(wParam)) == EN_CHANGE) && (GetDlgItemInt(g_hWnd, id, NULL, FALSE) > (id == IDE_THREAD ? (UINT)2000 : (UINT)20)))
            SetDlgItemInt(g_hWnd, id, id == IDE_THREAD ? 2000 : 20, FALSE);
        break;
    case IDC_USERANGE:
    case IDC_USELIST:
        if (SendDlgItemMessage(g_hWnd, id, BM_GETCHECK, 0, 0))
        {
            SendDlgItemMessage(g_hWnd, id, BM_SETCHECK, 0, 0);
            if (!SendDlgItemMessage(g_hWnd, (id == IDC_USELIST) ? IDC_USERANGE : IDC_USELIST, BM_GETCHECK, 0, 0))
                SendDlgItemMessage(g_hWnd, (id == IDC_USELIST) ? IDC_USERANGE : IDC_USELIST, BM_SETCHECK, 1, 0);
        }
        else
            SendDlgItemMessage(g_hWnd, id, BM_SETCHECK, 1, 0);
        break;
    case IDC_GETBANNER:
    case IDC_KEEPHISTORY:
        if (CheckMenuItem(g_hMenu, id, MF_CHECKED) == MF_CHECKED)
            CheckMenuItem(g_hMenu, id, MF_UNCHECKED);
    ...
    case IDC_SAVE:
        ...
        strTemp[0] = '\0';
        if (id == IDC_LOAD) strcpy(strTemp, "PortList.txt");
        ofn.nMaxFile = 256;
        ...
        if (id == IDC_LOAD)
        {
            ...
            if (GetOpenFileName(&ofn))
                SendMessage(g_hwndStatus, SB_SETTEXT, 0, (LPARAM)(LoadResult(ofn.lpstrFile) ? "Succeeded!" : "Failed!"));
        }
        else
        {
            ...
            if (GetSaveFileName(&ofn))
                SendMessage(g_hwndStatus, SB_SETTEXT, 0, (LPARAM)(SaveResult(ofn.lpstrFile, ListView_GetSelectedCount(g_hwndListView)) ? "File Saved" : "Save Failed"));
        }
        break;
    case IDC_DEL:
    ...
    case IDC_SORTPORT:
    case IDC_SORTBANNER:
        ListView_SortItems(g_hwndListView, CompareFunc, id - 514);
        break;
    case IDC_TELNET:
        for (i = 0; i < nItemCount; i++)
        {
...
#define lpmsg ((LPNMHDR)lParam)
    ...
    case LVN_DELETEITEM:
    {
        ...
        ITEMINFO* pItem = (ITEMINFO*)pnmv->lParam;
        if (pItem)
        {
            if (pItem->lpstrBanner)
            {
                LocalFree(pItem->lpstrBanner);
            }
            LocalFree(pItem);
        }
        break;
    ...
    case TTN_NEEDTEXT:
        ...
        break;
    case TTN_SHOW:
        SendMessage(g_hwndStatus, SB_SETTEXT, 0, (LONG_PTR)c_szToolTip[GetDlgCtrlID((HWND)lpmsg->idFrom) - 400]);
        break;
    case TTN_POP:
        SendMessage(g_hwndStatus, SB_SETTEXT, 0, (LONG_PTR)c_szWelcome);
        break;
    case LVN_COLUMNCLICK:
    {
        ...
        ListView_SortItems(g_hwndListView, CompareFunc, pnm->iSubItem);
        break;
    }
    case LVN_GETDISPINFO:
    {
        ...
        {
            ITEMINFO* pItem = (ITEMINFO*)(pnmv->item.lParam);
            if (pItem)
            {
                if (pnmv->item.iSubItem == 0)
                {
                    struct in_addr ia;
                    ia.s_addr = htonl(pItem->dwAddr);
                    lstrcpy(pnmv->item.pszText, inet_ntoa(ia));
                }
                else if (pnmv->item.iSubItem == 1)
                {
                    sprintf(pnmv->item.pszText, "%d", pItem->wPort);
                }
                else
                {
                    if (pItem->lpstrBanner)
...
void WINAPI On_DrawItem(WPARAM wParam, LPARAM lParam)
{
    ...
    int iState = lpDraw->itemState & ODS_SELECTED;
    int idx = 0;
    ...
    if (wParam == IDC_SCAN)
    {
        if (lpDraw->itemState & ODS_DISABLED) idx = 2;
        else idx = iState;
        SelectObject(hdcMem, g_hBitmap[idx]);
        TextOut(hdcMem, 12 + idx, 5 + idx, "Scan", 4);
    }
    else if (wParam == IDC_STOP)
    {
        if (lpDraw->itemState & ODS_DISABLED) idx = 5;
        else idx = 3 + iState;
        SelectObject(hdcMem, g_hBitmap[idx]);
        TextOut(hdcMem, 12 + iState, 5 + iState, "Stop", 4);
    }

    BitBlt(lpDraw->hDC, lpDraw->rcItem.left, lpDraw->rcItem.top,
        lpDraw->rcItem.right - lpDraw->rcItem.left,
        lpDraw->rcItem.bottom - lpDraw->rcItem.top, hdcMem, 0, 0, SRCCOPY);
    DeleteDC(hdcMem);
}

static void Helper_ParsePortList(char* strList, int startPort, int endPort, BOOL useRange)
{
    char* pPoint1 = strList;
    char* pPoint2;
    int j;
    nPortCount = 0;

    while (*pPoint1 && (nPortCount < 100))
    {
        j = atoi(pPoint1);
        if (j && j < 65535 && (j < startPort || j > endPort || !useRange))
        {
            nPort[nPortCount] = j;
            nPortCount++;
        }
...
static void Helper_WaitAndCleanUp(DWORD_PTR dwStartTime)
{
    ...
    while (nThreadNum > 0)
    {
        if (!(dwWaitIP % 500000))
        {
            ...
            for (intINT = 0; intINT < (int)((dwWaitIP / 500000) % 7); intINT++)
                strcat(strString, ">");

            SendMessage(g_hwndStatus, SB_SETTEXT, (WPARAM)0, (LPARAM)strString);

            if (dwWaitIP == 3000000) j = 0;
            else if (dwWaitIP == 0)  j = 1;
        }
        if (j) dwWaitIP++; else dwWaitIP--;
    }

    SendMessage(g_hwndStatus, SB_SETTEXT, (WPARAM)0, isStopForce ? (LPARAM)"Aborted" : (LPARAM)"Completed");
    sprintf(strString, "Cost %lldms", GetTickCount64() - dwStartTime);
...
void WINAPI StartScan()
{
    ...
    char strTemp[600];
    char strString[600];
    IN_ADDR iaServerLocal;
    DWORD_PTR dwStartTime, dwCurIP;
    BOOL isStartScanRange = TRUE;

    nSkipHost = 0;
    nScannedPort = 0;
    isStopForce = FALSE;
    ZeroMemory(&thdINFO, 2000 * sizeof(THREADINFO)); 
    ...
    if (!intMAXThread) { SetDlgItemInt(g_hWnd, IDE_THREAD, 50, 0); intMAXThread = 50; }

    nTimeOut = GetDlgItemInt(g_hWnd, IDE_TIMEOUT, NULL, 0);
    if (!nTimeOut) { SetDlgItemInt(g_hWnd, IDE_TIMEOUT, 3, 0); nTimeOut = 3; }

    SendDlgItemMessage(g_hWnd, IDIP_STARTIP, IPM_GETADDRESS, 0, (LPARAM)&dwStartIP);
    SendDlgItemMessage(g_hWnd, IDIP_ENDIP, IPM_GETADDRESS, 0, (LPARAM)&dwEndIP);
    if (dwStartIP > dwEndIP) {
        dwCurIP = dwStartIP; dwStartIP = dwEndIP; dwEndIP = dwCurIP;
    ...
    if (isUsePortRange) {
        ...
        if (nStartPort > nEndPort) { intINT = nStartPort; nStartPort = nEndPort; nEndPort = intINT; }
        if (!nStartPort) {
            if (!nEndPort) isUsePortRange = FALSE;
            else { nStartPort = nEndPort; SetDlgItemInt(g_hWnd, IDE_STARTPORT, nEndPort, 0); SetDlgItemInt(g_hWnd, IDE_ENDPORT, nEndPort, 0); }
        }
    }

    if (isUsePortList) {
        GetDlgItemText(g_hWnd, IDE_PORTLIST, strString, 599);
        Helper_ParsePortList(strString, nStartPort, nEndPort, isUsePortRange);
        if (!nPortCount) isUsePortList = FALSE;
    }
    ...
    if (isUsePortList || isUsePortRange)
    {
        ...
        j = isUsePortList ? nPort[0] : nStartPort;

        if (isUseResultIP) {
            dwCurIP = 0;
            for (dwStartIP = 0, dwEndIP = 0; (int)dwEndIP < nItemCount; dwEndIP++)
                if (ListView_GetCheckState(g_hwndListView, dwEndIP)) dwStartIP++;
        }
        else {
            dwCurIP = dwStartIP;
            if (!isKeepHistroy) {
        ...
        {
            if (isStopForce) break;

            if (((dwCurIP % 256) == 0 || (dwCurIP % 256) == 255) && !isUseResultIP) {
                nSkipHost++; dwCurIP++; continue;
            }
            ...
            if (nThreadNum < intMAXThread)
            {
                ...
                nThreadNum++; // Optimistic Locking (preoccupy)
            }
            LeaveCriticalSection(&g_cs);

            if (bCanCreateThread) {
                for (intINT = 0; intINT < intMAXThread; intINT++)
                    if (thdINFO[intINT].wPort == 0) break;

                if (isUseResultIP) {
                    while (dwCurIP < dwEndIP) {
                        if (ListView_GetCheckState(g_hwndListView, dwCurIP)) break;
                        dwCurIP++;
                    }
                    if (dwCurIP == dwEndIP) {
                        EnterCriticalSection(&g_cs); nThreadNum--; LeaveCriticalSection(&g_cs);
                    }
                ...
                // 64KB stack per thread
                HANDLE hThread = CreateThread(NULL, 65536, (LPTHREAD_START_ROUTINE)ScanThread, (LPVOID)intINT, STACK_SIZE_PARAM_IS_A_RESERVATION, &dwThreadID);
                if (hThread != NULL)
                {
                    CloseHandle(hThread);
                }
                else
                {
                    ...
                    thdINFO[intINT].wPort = 0;
                    Sleep(10);
                    continue;
                }

                iaServerLocal.s_addr = htonl(thdINFO[intINT].dwAddr);
                sprintf(strString, "Scanning %s:%d", inet_ntoa(iaServerLocal), j);
                SendMessage(g_hwndStatus, SB_SETTEXT, (WPARAM)0, (LPARAM)strString);
                ...
                if (isUsePortRange) {
                    if (isUsePortList) {
                        if (k == nPortCount - 1) {
                            if (isStartScanRange) {
                                j = nStartPort; isStartScanRange = FALSE;
                            }
                            else {
                                if (j == nEndPort) {
                                    j = nPort[0]; k = 0; isStartScanRange = TRUE; dwCurIP++;
                                }
                                else j++;
                            }
                        }
                        ...
                        if (j == nEndPort) { j = nStartPort; dwCurIP++; }
                        else j++;
                    }
                }
                else {
                    if (k == nPortCount - 1) { k = 0; dwCurIP++; }
                    else k++;
                    j = nPort[k];
                }
            }
        ...
        Helper_WaitAndCleanUp(dwStartTime);
    }
    else
    {
...
void WINAPI ScanThread(int intTread)
{
    ...
    else
    {
        ...
        if (value && value != SOCKET_ERROR)
        {
            ...
            switch (pItem->wPort) {
            case 21: lvi.iImage = 1; break; case 23: lvi.iImage = 2; break;
            case 25: lvi.iImage = 3; break; case 79: lvi.iImage = 4; break;
            case 80: lvi.iImage = 5; break; case 110: lvi.iImage = 6; break;
            case 135: lvi.iImage = 7; break; case 139: lvi.iImage = 8; break;
            case 1080: case 8080: lvi.iImage = 9; break;
            case 1433: lvi.iImage = 10; break; case 3389: lvi.iImage = 11; break;
            default: lvi.iImage = 0;
            }
...
_Cleanup: 
    ...
    sprintf(strString, "Completed: %d%%", (int)(nScannedPort * 100 / 
        ((isUseResultIP ? dwStartIP : (dwEndIP - dwStartIP + 1 - nSkipHost)) 
            * (nPortCount * isUsePortList + (nEndPort - nStartPort + 1) * isUsePortRange))));
...
BOOL WINAPI SaveResult(char* strFileName, BOOL isSaveSelected)
{
    ...
    {
        if (isSaveSelected && !ListView_GetItemState(g_hwndListView, i, LVIS_SELECTED)) continue;

        char ip[32], port[10], banner[256];
        ListView_GetItemText(g_hwndListView, i, 0, ip, sizeof(ip));
        ListView_GetItemText(g_hwndListView, i, 1, port, sizeof(port));
        ...
        if (ListView_GetItem(g_hwndListView, &lvi))
        {
            ...
            if (pItem && pItem->lpstrBanner) {
                strncpy(banner, pItem->lpstrBanner, sizeof(banner) - 1);
                banner[255] = '\0';
            }
        }

        fprintf(fp, "%s:%s", ip, port);
        if (strlen(banner) > 0) fprintf(fp, "\t%s", banner);
        fprintf(fp, "\n");
    }
    fclose(fp);
    return TRUE;
...
void WINAPI ResolveDomain()
{
    ...
    SendMessage(g_hwndStatus, SB_SETTEXT, (WPARAM)0, isStopForce ? (LPARAM) "Aborted" : (LPARAM) "Completed");
    sprintf(strString, "Cost %lldms", GetTickCount64() - dwStartIP);
    SendMessage(g_hwndStatus, SB_SETTEXT, (WPARAM)1, (LPARAM)strString);
    nThreadNum = 0;
    SendMessage(g_hwndStatus, SB_SETTEXT, (WPARAM)2, (LPARAM) "Active thread: 0");
    EnableWindow(GetDlgItem(g_hWnd, IDC_SCAN), 1);
